...
async function importNextTendersWithAuftraggeber() {
  try {
    console.log('Starting tender import with Auftraggeber information...');
    
    // Read the Vertriebsprozess file
    ...
    console.log(`Found ${tenderData.length} rows in Vertriebsliste`);
    ...
    for (let i = 3; i < tenderData.length && processedCount < maxToProcess; i++) {
      ...
      // Extract data from the row based on the structure we saw
      const oppId = row[2]; // Opp-ID column
      const art = row[1]; // Art column
      const kunde = row[3]; // Kunde column (Auftraggeber)
      ...
      const oppPartner = row[12]; // Opp Partner column
      ...
      // Use angefragteLeistung as title if available
      const title = angefragteLeistung || kunde || 'Untitled Tender';
      
      // Determine status based on requirements
      const finalStatus = determineStatus(status, art);
      
      // Create organisation if needed (Auftraggeber)
      let organisation = null;
      if (kunde) {
        organisation = await findOrCreateOrganisation(kunde);
      }
      
      // Create organisation role for Auftraggeber
      const organisationRole = await findOrCreateOrganisationRole('Auftraggeber');
      
      console.log(`Processing tender ${oppId}: ${title} (Auftraggeber: ${kunde})`);
      ...
      // Create organisation relationship if organisation exists (Auftraggeber)
      ...
      processedCount++;
      console.log(`Successfully created tender: ${title} (Opp-ID: ${oppId}, Auftraggeber: ${kunde})`);
    }
    
    console.log(`Import completed! Processed ${processedCount} tenders with Auftraggeber information.`);
...
// Run the import
importNextTendersWithAuftraggeber(); 