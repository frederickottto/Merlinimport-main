...
async function debugDetailView() {
  try {
    ...
    if (tender) {
      ...
      // Simulate the API processing (like in tender.ts)
      ...
      const transformedStatus = statusMap[processedTender.status] || String(processedTender.status);
      console.log('\n=== TRANSFORMED STATUS ===');
      console.log('Original:', processedTender.status);
      console.log('Transformed:', transformedStatus);
      
      // Test what DetailView would receive
      ...
      const statusField = {
        ...
        position: 3,
        transform: (value) => {
          console.log('Transform function called with value:', value, 'type:', typeof value);
