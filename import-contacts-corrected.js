...
async function importContactsCorrected() {
  console.log('🚀 Starte Import der Organisations-Kontakte (korrigiert)...\n');
  ...
  try {
    ...
    for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
      ...
      if (row[1] && row[1] !== '#') {
        // Extract organization number from the first column
        const orgNumber = row[0] ? parseInt(row[0]) : null;
    ...
    // Show first few organizations with their numbers
    console.log('\n📋 Erste 10 Organisationen mit Nummern:');
    organizations.slice(0, 10).forEach((org, index) => {
      console.log(`   ${index + 1}. #${org.number}: ${org.name} (Zeile ${org.rowIndex})`);
    });
    
    // Show first few contacts and their numbers
    console.log('\n📋 Erste 10 Kontakte mit Nummern:');
    contacts.slice(0, 10).forEach((contact, index) => {
      console.log(`   ${index + 1}. #${contact.number}: ${contact.firstName} ${contact.lastName} (Zeile ${contact.rowIndex})`);
    });
    ...
    for (const contact of contacts) {
      try {
        ...
        // Find the corresponding organization by matching the contact number
        // with the organization's number (not row number)
        const matchingOrg = organizations.find(org => {
          return org.number === contactNumber;
        });
        ...
        console.log(`✅ Kontakt importiert: ${contact.firstName} ${contact.lastName} (${contact.position || 'keine Position'}) → ${matchingOrg.name} (#${matchingOrg.number})`);
        importedCount++;
