...
function convertNiveauToText(niveau) {
  ...
  if (isNaN(niveauNum)) return niveau; // Return original if not a number
  ...
  switch (niveauNum) {
    ...
    case 3:
      return "Experte";
    case 4:
      return "Experte"; // Level 4 is also expert level
    default:
      return niveau; // Return original if unknown
  }
}

function transformEmployeeData(excelData) {
  console.log(`🔍 Transformiere Employee Data:`, Object.keys(excelData));
  const transformed = {};
  for (const [key, value] of Object.entries(excelData)) {
    const mappedField = fieldMappings.Employee[key];
    if (mappedField) {
      console.log(`   📋 Mapping: "${key}" -> "${mappedField}" = "${value}"`);
      if (mappedField === "contractStartDate") {
        if (typeof value === 'number') {
          ...
          if (!isNaN(parsed.getTime())) {
            transformed[mappedField] = parsed;
          }
        }
      } else if (mappedField === "experienceIt" || mappedField === "experienceIs" || mappedField === "experienceItGs" || mappedField === "experienceGps") {
        transformed[mappedField] = parseInt(value) || 0;
      } else {
        transformed[mappedField] = value;
      }
    } else {
      console.log(`   ⚠️  Kein Mapping für: "${key}" = "${value}"`);
    }
  }
  console.log(`   ✅ Transformed result:`, Object.keys(transformed));
  return transformed;
}
  ...
  for (const file of files) {
    ...
    if (workbook.SheetNames.includes('Mitarbeiter')) {
      ...
      if (transformed.location) {
        console.log(`🔍 Verarbeite Standort: "${transformed.location}"`);
        
        // First try to find existing location by city name
        let location = await prisma.location.findFirst({
          where: { city: transformed.location }
        });
        
        if (!location) {
          // If not found, create a new location with the city name
          console.log(`📍 Erstelle neuen Standort: ${transformed.location}`);
          ...
          console.log(`📍 Verwende bestehenden Standort: ${transformed.location} (ID: ${location.id})`);
        }
        
        locationConnect = { locationIDs: location.id };
        console.log(`📍 Location Connect: ${JSON.stringify(locationConnect)}`);
      } else {
        console.log(`⚠️  Kein Standort in transformed data gefunden`);
      }
        ...
        if (Object.keys(transformed).length > 0 || model === 'EmployeeProjectActivities') {
          try {
            if (model === 'ProfessionalBackground') {
              ...
              // Check if we already imported skills for this employee
              const existingSkills = await prisma.employeeSkills.findMany({
                where: { employeeIDs: { has: currentEmployee.id } }
              });
              
              if (existingSkills.length > 0) {
                console.log(`⏭️  Skills bereits für ${currentEmployee.pseudonym} importiert`);
                continue;
              }
              ...
              for (const row of dataRows) {
                ...
                try {
                  let skill = await prisma.skills.findFirst({ 
                    where: { title: transformed.skillTitle } 
                  });
                  
                  if (!skill) {
                    skill = await prisma.skills.create({ 
                      data: { 
                        title: transformed.skillTitle,
                        type: 'Technisch',
                  ...
                  await prisma.employeeSkills.create({
                    data: {
                      employeeIDs: [currentEmployee.id],
                      skillIDs: skill.id,
                      niveau: transformed.niveau !== undefined && transformed.niveau !== null ? convertNiveauToText(transformed.niveau) : null
                    }
                  });
                  skillImportCount++;
                } catch (error) {
              ...
              if (skillImportCount > 0) {
                console.log(`✅ ${skillImportCount} Skills für ${currentEmployee.pseudonym} importiert`);
              }
