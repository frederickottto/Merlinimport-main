export const dynamic = 'force-dynamic';

import { Skeleton } from "@/components/ui/skeleton";
import { DataTable } from "@/components/table/table";
import { api, HydrateClient } from "@/trpc/server";
import { tenderColumns } from "@/server/controllers/tender/table-config";
import { T_Tender } from "@/server/controllers/tender/schema";

const Page = async () => {
	try {
		const data = await api.tenders.all() as T_Tender[];

		if (!data) {
			return (
				<div className="flex flex-col space-y-4">
					<div className="text-gray-400">No Data Found!</div>
					<Skeleton className="w-[500px] h-[20px] rounded-full" />
					<Skeleton className="w-[500px] h-[20px] rounded-full" />
					<Skeleton className="w-[300px] h-[20px] rounded-full" />
					<Skeleton className="w-[200px] h-[20px] rounded-full" />
				</div>
			);
		}

		return (
			<HydrateClient>
				<div className="space-y-6">
					<div className="flex items-center justify-between">
						<h1 className="text-3xl font-bold tracking-tight">Tenders</h1>
					</div>
					<DataTable 
						data={data} 
						columns={tenderColumns}
						tabValue="tenders"
						viewMode="navigation"
					/>
				</div>
			</HydrateClient>
		);
	} catch (error) {
		console.error("Error fetching tenders data:", error);
		return (
			<div className="text-red-500">
				Error fetching data. Please try again later.
			</div>
		);
	}
};

export default Page;
